...
if ($chargeNow) {
    foreach ($chargeNow as $subscribe) {
        ...
        if ($transaction) {
            ...
            $subject = "[PAGAMENTO CONFIRMADO] Obrigado por assinar o BtApp";
            $body = $view->render("mail", [
                "subject" => $subject,
                "message" => "<h3>Obrigado {$user->first_name}!</h3><p>Estamos passando apenas para agradecer por você ser um assinante BtApp {$plan->name}.</p><p>Sua fatura deste mês venceu hoje e já está paga de acordo com seu plano. Qualquer dúvida estamos a disposição.</p>"
            ]);
...
if ($chargeDays) {
    foreach ($chargeDays as $subscribe) {
        ...
        if ($transaction) {
            ...
            $subject = "[ASSINATURA CANCELADA] Sua conta BtApp agora é FREE";
            $body = $view->render("mail", [
                "subject" => $subject,
